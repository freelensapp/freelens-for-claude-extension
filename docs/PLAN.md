...
Status: **draft for approval** (issue [#7](https://github.com/freelensapp/freelens-for-claude-extension/issues/7))
...
### D2. Authentication and terms-of-service posture (needs explicit approval)
...
A later "power mode" preference may optionally enable `Bash` + `kubectl`
with the cluster's kubeconfig exported — off by default.
...
- Registered via `clusterPages` + `clusterPageMenus` ("Claude" entry with
  icon), which gives a page instance per cluster frame — matching the
  per-cluster context model.
...
for analysis workflows, prompt shortcuts/slash commands.

## Open questions for approval

1. **D2 posture**: proceed as a frontend to the user's own Claude Code, and
   contact Anthropic before marketplace publication?
2. **D4 safety default**: built-in `Bash`/filesystem tools disallowed, typed
   MCP tools only (with a possible later opt-in power mode)?
3. **M0 scope**: read-only tools only for the PoC?
4. Extension display name for the cluster menu ("Claude"?).
